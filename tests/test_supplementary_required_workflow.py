...
class OrganizationRequiredWorkflowTests(unittest.TestCase):
    ...
    def test_permanent_verifier_awaits_protected_evidence_without_a_rerun_race(
        ...
        self.assertIn("bad_terminal_jobs", permanent)
        self.assertIn('and .conclusion != "success"', permanent)
        self.assertIn("allowed_skips", permanent)
        self.assertIn(
        ...
        self.assertLess(
            producer_loop.index("bad_terminal_jobs"),
    ...
    def test_permanent_verifier_rejects_every_unexpected_terminal_job(self) -> None:
        ...
        self.assertEqual(
            [job["name"] for job in evaluate("bad_terminal_jobs")],
            [
                "Request protected verifier re-evaluation / "
                "Diagnose Release-App reusable context and fail closed",
                "cancelled job",
        ...
        self.assertEqual(
            [job["name"] for job in evaluate("allowed_skips")],
            [
        ...
        for predicate, passing, failing in (
            ("length == 0 or error(tojson)", [], evaluate("bad_terminal_jobs")),
            (
                "(length <= 2 and "
                "(map(.name) | unique | length) == length) "
                "or error(tojson)",
                evaluate("allowed_skips"),
